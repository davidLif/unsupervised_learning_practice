...
from sklearn.metrics import mutual_info_score, silhouette_score, silhouette_samples

from FinalProject.clustering import apply_clustering, algo_types_clustering_params
from FinalProject.dim_reduction import apply_dim_reduction
from FinalProject.anomaly_detection import apply_anomaly_detection
from FinalProject.statistic_tests import StatTester

external_variables = ["dAge", "dHispanic", "iYearwrk", "iSex"]
clustering_algs = list(
    algo_types_clustering_params.keys())  # ["KMeans", "KModes", "Hierarchical", "DBSCAN", "GaussianMixture"]
anomaly_detect_algs = []
dim_reduction_algs = ["PCA", "CMDS", "ISO", "LLE", "EigenMaps"]
stats_dir = "FinalProject/stats"
...
class Evaluator:
    def __init__(self, data, num_of_iterations_for_statistical_analysis=10, eval_on_test=False, algs_type="clustering"):
        ...
        if self.algs_type == "clustering":
            self.algs = list(algo_types_clustering_params.keys())
    ...
    def apply_algs_with_all_hyper_params_configs(self):
        self.data_subsets, self.external_tags = get_subsets(self.data, self.num_of_iterations_for_statistical_analysis)
    ...
    def extract_scores(self, alg, combo_config, scores_to_extract=[]):
        ...
        for ((train_d, test_d), (train_tags, test_tags)) in tqdm(zip(self.data_subsets, self.external_tags)):
            ...
            n_labels, model = run_single_algo(x, alg, combo_config)
...
def load_data():
    ...
    return data_df.sample(10000, random_state=0)  # train_data, test_data
...
if __name__ == "__main__":
    data = load_data()
    clstring_evaluator = Evaluator(data, algs_type="clustering")
    clstring_evaluator.algs_evaluation_pipeline()
